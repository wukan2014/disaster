...
dashboard_file = open('flood-5w1h.html', 'w', encoding='utf8')
...
for i in range(len(match_cont)) :
    ...
    if isDisaster:
        ...
        donut_csv = create_donut_csv(df.ix[i]['title'],df.ix[i]['content'])
        ...
        message+="""
	    		<tr>
	    			<td><p>Flood</p><div class="donut" data-counts='""" + donut_csv + """'></div></td>
	    			<td></td>
	    			<td><a target="_blank" href='""" + str(df.ix[i]['url']) + """'>""" + str(df.ix[i]['url']) + """</a</td>
	    			<td>""" + str(location[df.ix[i]['title']])[1:-1] + """</td>
	    			<td>""" + str(df.ix[i]['publication_date']) + """</td>
					<td><p class="hover_label">How<span class="not_hover">""" + str(extract_number_sentences(df.ix[i]['content']))[1:-1] + """</span></p></td>
				</tr>"""
...
filename = 'file:///Users/samikanza/Documents/ChinaDatathon/disaster/chord_diagram/' + 'flood-5w1h.html'
